...
xSliceCoords = linspace(0,2,5);
ySliceCoords = linspace(0,2,5);
zSliceCoords = linspace(0,2,5);
% xSliceCoords = 1;
% ySliceCoords = 1;
% zSliceCoords = 1;
...
% Use symbol PlotStyle(k) at refinement level k.
  ...
  y0 = 0.0;
  z0 = 0.0;
  ScatterStyle = strvcat('o','x','.','s','v','^');
  plotstyle = 'o';

% For PlotType = z5 (Isosurface plots)
% Note:  Lengths of SurfTransparency and SurfColors must greater than or
%        equal to the length of SurfValues..

  IsosurfValues    =  [0.1  0.5];     % Plot surfaces at q = surfValue(i).

  IsosurfAlphas    =  [0.1   1];     % Transparency of each surface
                                          % (0=clear; 1=opaque)
